...
```{r daily-plot, fig.width=12, fig.height=6.8, out.width='100%'}
long <- yields |>
...
```{r macro-plot, fig.width=12, fig.height=6.3, out.width='100%'}
macro_long <- macro |>
...
```{r response-plot, fig.width=12, fig.height=6.3, out.width='100%'}
...
document.addEventListener("DOMContentLoaded", function () {
  const macroSection = document.getElementById("graphic-2-policy-rate-inflation-and-unemployment");
  const combineSection = document.getElementById("combine-daily-yields-with-policy-markers");
  if (macroSection && combineSection) {
    combineSection.parentNode.insertBefore(macroSection, combineSection);
  }

  const macroToc = document.querySelector('a[href="#graphic-2-policy-rate-inflation-and-unemployment"]');
  const combineToc = document.querySelector('a[href="#combine-daily-yields-with-policy-markers"]');
  if (macroToc && combineToc) {
    combineToc.closest("ul").insertBefore(macroToc.closest("li"), combineToc.closest("li"));
  }
});
</script>
